...
function addPrePostMargin(
  ...
  const lenWMargin = Math.max(
    Math.trunc(Math.ceil(start + length)) + margin - startWMargin,
    0
  );
...
export function createSpectrogramSliceArgs(
  ...
  const [leftWMarginClipped, widthWMarginClipped, leftMargin, rightMargin] =
    addPrePostMargin(leftF64, widthF64, nFrames, marginPx);
  ...
  const [topWMarginClipped, heightWMarginClipped, topMargin, bottomMargin] =
    addPrePostMargin(topF64, heightF64, nFreqs, marginPx);
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (skip) 
        ...
        mipmaps[mipmaps.length - 1].push([widthRounded, heightRounded]);
